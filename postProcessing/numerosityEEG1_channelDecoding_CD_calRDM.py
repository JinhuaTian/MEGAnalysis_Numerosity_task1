...
for subjid in subjList:
    accs = np.zeros([session,decodingNum,decodingNum,decodingNum,decodingNum,tpoints]) # sessions(12) x train data x label x test data x label x tpoints
        ...
        for tp in range(tpoints):
            for trainDat in range(2): # 2 training datasets
                for trainLab in range(2): # 2 labels for each data sets
                    ...
                    model.fit(trainData[trainDat][:,:,tp],trainLabel[trainDat][trainLab])
                    for testDat in range(2): # 2 testing datasets
                        for testLab in range(2): # 2 testing label
                            acc = model.score(testData[testDat][:,:,tp],testLabel[testDat][testLab]) # sub,time,RDMindex,fold,repeat # t,re,foldIndex,RDMindex
                            # save acc
                            accs[sess,trainDat,trainLab,testDat,testLab,tp] = acc 
    ...
    # save MEG RDM
    fileName = pj(rootDir, 'crossDecoding12x'+str(newSamplingRate)+'hz_'+ subjid +'.npy')
