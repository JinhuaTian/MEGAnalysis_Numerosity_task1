...
def transLabel(events):
    ...
    return events
'''
# segment event list to 9 events [event1,event2,ect.]
def segLabel(events):
    eventsList = []
    for i in range(1,eventNum+1):
        eventsList.append(events[events[:,2]==i,:])
    return eventsList
'''
...
evokedList = []
epochs_all1.events = transLabel(epochs_all1.events)
for i in range(1,eventNum+1):
    epochDat = epochs_all1[str(i)].average().crop(tmin=tmin,tmax=tmax,include_tmax=True)
    evokedList.append(epochDat)
evokeds = {}
for i in range(eventNum):
    evokeds[eventName[i]]=evokedList[i]
# visulize evoked results
mne.viz.plot_compare_evokeds(evokeds,picks=picks,combine='mean',legend='upper right')

epochs_all2 = mne.concatenate_epochs(epochs_list2)
evokedList = []
epochs_all2.events = transLabel(epochs_all2.events)
for i in range(1,eventNum+1):
    epochDat = epochs_all2[str(i)].average().crop(tmin=tmin,tmax=tmax,include_tmax=True)
    evokedList.append(epochDat)
evokeds = {}
for i in range(eventNum):
    evokeds[eventName[i]]=evokedList[i]
...
Evoked_all2=epochs_all2.average()
Evoked_all2.plot()
dd2 = Evoked_all2.plot_topomap()
dd2.savefig('toponum.png')
'''
